...
def generate_data(n: int = 200):
    ...
    X = (np.random.random((n, 3)) - 0.5) * 10  # 统一在 [-5, 5] 采样
    y = f(X)
...
def train(X_tr : torch.Tensor, y_tr : torch.Tensor, 
    ...
    for i in trange(epochs) :
        ...
        tqdm.write(f"Epoch {i + 1:>3}: loss = {loss_sum / len(dataset):.6f}")

        loss_hist.append(loss_sum / len(dataset))
...
if __name__ == "__main__" :
    init_random()
    train(*generate_data(n=2000), epochs=100)