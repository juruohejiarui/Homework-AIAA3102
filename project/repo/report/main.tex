...
\input{../report_assets/tables/table_01_dataset_split_statistics.tex}
...
\input{../report_assets/tables/table_02_baseline_reference_comparison.tex}
...
\input{../report_assets/tables/table_03_five_ticket_summary.tex}
\input{../report_assets/tables/table_04_dev_metric_comparison.tex}
\input{../report_assets/tables/table_05_heldout_metric_comparison.tex}
\input{../report_assets/tables/table_06_confusion_matrix_summary.tex}
\input{../report_assets/tables/table_07_prediction_transitions.tex}
...
\input{../report_assets/tables/table_08_normalization_experiments.tex}
...
\input{../report_assets/tables/table_09_shortcut_shallow_feature_audit.tex}
...
\textbf{Hypotheses and lever.} Ticket 4 held the raw-text TF--IDF representation fixed while evaluating 61 thresholds from 0.20 through 0.80, Logistic Regression $C\in\{0.25,0.5,1,2,4\}$, balanced class weights at $C=1$ and threshold 0.50, and one LinearSVC comparator at $C=1$. Lower thresholds and balanced weighting were expected to raise recall at a precision cost; regularization and LinearSVC could change sparse-feature separation.

\input{../report_assets/tables/table_10_decision_rule_model_comparison.tex}
...
\textbf{Dev evidence and decision.} Figure~\ref{fig:threshold} shows the expected precision--recall exchange across the baseline threshold sweep. The best threshold-only candidate was 0.47 with F1 0.749407. The best regularization-only candidate was $C=2$ with F1 0.750403. LinearSVC achieved 0.740859. Balanced Logistic Regression achieved the largest declared dev F1, 0.752085, with precision 0.746988 and recall 0.757252 (Table~\ref{tab:model_comparison}). It fixed 42 baseline FNs and introduced 48 FPs. It was frozen at $C=1$, balanced weights, and inclusive threshold 0.50.
...
\input{../report_assets/tables/table_11_data_quality_audit_summary.tex}
...
\input{../report_assets/tables/table_12_representative_case_analysis.tex}
...
\input{../report_assets/tables/table_13_difficulties_and_solutions.tex}
...
\input{../report_assets/tables/table_14_ai_usage_and_verification.tex}
...
Ticket 2's full normalization table is Table~\ref{tab:normalization}. Mention placeholdering was negative; punctuation-to-space was only marginally positive; hashtag stripping, casefolding, and emoji placeholdering were label-equivalent to raw text. Ticket 3's full ten-row comparison is Table~\ref{tab:shortcut_audit}. The isolated keyword, location, length, and shallow families were all weaker than text-only, while the rich model's visible gain failed robustness. Ticket 4's complete eight-row model table is Table~\ref{tab:model_comparison}; $C=0.25$ and $C=0.5$ hurt, LinearSVC did not win, and both threshold 0.47 and $C=2$ lost to balanced weighting under the declared dev criterion. These are bounded comparisons on one split, not exhaustive searches.
...
All report tables and figures were generated by \path{scripts/generate_report_assets.py}. The exact source list for each asset is \path{report_assets/asset_manifest.csv}, and the 196 assertion outcomes are in \path{report_assets/validation_report.json}. Central frozen recipes, hashes, archived prediction paths, expected metrics, and transition counts are in \path{configs/frozen_decisions.json}. Run commands remain in per-experiment \texttt{run\_command.txt} files rather than being pasted into this report.
